...
appendInfo <- toupper(json_data$config$transcriptome$transcriptomeQC$coveriance)=="TRUE"
appendInfoFile <- json_data$config$transcriptome$transcriptomeQC$coveriance_file
...
library(reshape2)
library(tidyverse)
ROSMAP_tpm <- read.csv(csvfile,header = TRUE,row.names = 1)
keep_genes_idx <- (rowMeans(ROSMAP_tpm>0.1)>0.2) 
ROSMAP_tpm = ROSMAP_tpm[keep_genes_idx,] #38471 * 367

if(appendInfo)
{
  ROSMAP_clinical_HC <- read.table(appendInfoFile,header = TRUE,stringsAsFactors = TRUE)
  rownames(ROSMAP_clinical_HC) <- ROSMAP_clinical_HC$mrna_id
  idx <- intersect(colnames(ROSMAP_tpm),rownames(ROSMAP_clinical_HC))
  ROSMAP_clinical_HC <- ROSMAP_clinical_HC[idx,]
  ROSMAP_tpm <- ROSMAP_tpm[,idx]
}

rawdata <- ROSMAP_tpm
RLEFilterLength <- RLEFilterPercent*ncol(ROSMAP_tpm)
DSFilter <- DSFilterPercent*ncol(ROSMAP_tpm)


#print raw data's statistic including RLE,D and hclust
#RLE
logtpm = log10(ROSMAP_tpm + 1e-4)
...
outputfile="samples.QC.plot.RLE.pdf"
par(mar=c(3,3,3,3))
...
hcphy = as.phylo(hc)
if(appendInfo && is.element("mrna_id",colnames(ROSMAP_clinical_HC)) && is.element("batch",colnames(ROSMAP_clinical_HC))){
 co = ROSMAP_clinical_HC$batch[match(hcphy$tip.label, ROSMAP_clinical_HC$mrna_id)]
 co = as.factor(co)
...
if(appendInfo && is.element("sex",colnames(ROSMAP_clinical_HC))){
  #reserved words:msex,mrna_id,mrna_id_batch,age_death,pmi
    ...
    d$Sex = ROSMAP_clinical_HC$sex[match(rownames(d), ROSMAP_clinical_HC$mrna_id)]
...
if(!appendInfo){
  print("with no coveriance information, the pipeline is ended")
  outliersIndex <- which(colnames(ROSMAP_tpm) %in% outliersList)
  if(length(outliersList)!=0)
  {
    ROSMAP_tpm <- ROSMAP_tpm[,-outliersIndex]
  }
  ...
  rm(m,sd)
  write.table(format(expr, digits=4,nsmall=4), file = "expression.normalization.xls", sep="\t", col.names = NA, quote=F,row.names = TRUE)
...
if(is.element("mrna_id",colnames(ROSMAP_clinical_HC)) || is.element("batch",colnames(ROSMAP_clinical_HC))){
  keep_genes_idx <- (rowSums(ROSMAP_tpm>0.1)>=10) 
  expr= ROSMAP_tpm[keep_genes_idx,]
  expr=log10(expr+1e-4)  # so row value of 0 will be -4 in the transformed value
  # outlier correction: quantile normalization with order preserved. Now TPM is changed to rank normalized gene expression.
  ...
  write.table(ROSMAP_clinical_RNAseqSamples, file = "ROSMAP_clinical_RNAseqQCedSamples.txt", sep="\t", col.names=TRUE, quote=F, row.names=FALSE)
}
